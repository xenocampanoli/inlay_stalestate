...
JsonValidators::JsonValidators(Json::Value* jvoP) : JVOP(jvoP) {
	std::string s = (*jvoP)["Name"];
...
//	std::cout << "trace 1 validateStringData " << std::to_string(JVOP[keyArg]) << "." << std::endl;
	std::string szs = std::to_string(JVOP[keyArg].size());
	std::cout << "trace 2 validateStringData " << szs << "." << std::endl;
	if ( JVOP[keyArg].size() >= minValue ) return;
